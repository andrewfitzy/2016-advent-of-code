...
class Task02(puzzleInput: List<String>) {
    ...
    fun solve(distanceFromStart: Int): Int {
        val favouriteNumber = input[0].toInt()
        val map = buildMap(Point(distanceFromStart, distanceFromStart), favouriteNumber)

        val startPoint = Point(1, 1)

        val filledArea = getFilledArea(startPoint, distanceFromStart, map)
        printSolution(filledArea, map)

        return filledArea.size
    }
    ...
    private fun getFilledArea(
        ...
        while (!deque.isEmpty()) {
            ...
            for (move in possibleMoves) {
                ...
                if (visited.contains(tmpPoint) ||
                    !isValidMove(tmpPoint, map) ||
                    step.route.size == distanceFromStart + 1
                ) {
                    continue
                }
